...
export function BottomNav({ active, onChange }: BottomNavProps) {
  return (
    <nav className="glass-strong rounded-2xl p-1 flex items-center gap-1">
      {tabs.map(({ id, icon: Icon, label }) => (
        ...
        >
          ...
          <span className={`text-[10px] font-display ${active === id ? 'text-primary font-semibold' : 'text-muted-foreground'}`}>
            {label}
          </span>
        </button>
      ))}
